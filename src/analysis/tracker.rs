...
fn data_kind_from_op(op: Opcode) -> DataKind {
    match op {
        ...
        Opcode::Lwa => DataKind::Word,
        Opcode::Ld | Opcode::Ldu | Opcode::Std | Opcode::Stdu => DataKind::Double,
...
mod ds_tests {
    ...
    fn fixture(memory: u32, target: u32, addi: bool) -> Result<ObjInfo> {
        let hi = target.wrapping_add(0x8000) >> 16;
        let mut words = vec![0x3d600000 | hi]; // lis r11, target@ha
        if addi {
            words.push(0x396b0000 | (target & 0xffff));
        }
        words.push(memory | if addi { 0 } else { target & 0xfffc });
        words.push(0x4e800020); // blr
        let data: Vec<u8> = words.into_iter().flat_map(u32::to_be_bytes).collect();
    ...
    #[test]
    fn ds_relocations_and_data_kinds() -> Result<()> {
        // lwa sign-extends a four-byte memory operand; ld/std access eight bytes.
        for (memory, kind) in [
            (0xe86b0000, ObjDataKind::Double),
            (0xe86b0001, ObjDataKind::Double),
            (0xe86b0002, ObjDataKind::Byte4),
            (0xf86b0000, ObjDataKind::Double),
            (0xf86b0001, ObjDataKind::Double),
        ] {
            for target in [0x82004260, 0x8200c260] {
                for addi in [false, true] {
                    let mut obj = fixture(memory, target, addi)?;
                    let mut vm = VM::new();
                    for offset in (0..obj.sections[0].size - 4).step_by(4) {
                        let address = 0x82100000 + offset;
                        let ins = crate::analysis::disassemble(&obj.sections[0], address).unwrap();
                        vm.step(&obj, SectionAddress::new(0, address), ins);
                    }
                    if memory & 3 == 1 {
                        assert_eq!(
                            vm.gpr[11].value,
                            GprValue::Address(RelocationTarget::Address(SectionAddress::new(
                                1, target
                            )))
                        );
                    }
                    ...
                    for (site, expected) in [
                        (0x82100000, ObjRelocKind::PpcAddr16Ha),
                        (0x82100004, ObjRelocKind::PpcAddr16Lo),
                    ] {
                        let reloc = obj.sections[0]
                            .relocations
                            .at(site)
                            .expect("missing relocation");
                        assert_eq!(reloc.kind, expected);
                        assert_eq!(reloc.addend, 0);
                        let symbol = &obj.symbols[reloc.target_symbol];
                        assert_eq!(symbol.address, target);
                        assert_eq!(symbol.data_kind, kind);
                    }
                    assert_eq!(obj.sections[0].relocations.iter().count(), 2);
                    let split = split_obj(&obj, None)?;
                    for write in [write_coff, write_link_coff] {
                        let bytes = write(&split[0])?;
                        let coff = object::File::parse(bytes.as_slice())?;
                        let text = coff.section_by_name(".text").unwrap();
                        let data = text.data()?;
                        let offset = if addi { 8 } else { 4 };
                        assert_eq!(
                            u32::from_be_bytes(data[offset..offset + 4].try_into()?),
                            memory
                        );
                        assert_eq!(
                            text.relocations()
                                .filter(|(_, r)| matches!(
                                    ...
                                    object::RelocationFlags::Coff {
                                        typ: object::pe::IMAGE_REL_PPC_REFHI
    ...
    #[test]
    fn ds_respects_blocked_and_invalid_targets() -> Result<()> {
        for memory in [0xe86b0000, 0xe86b0001, 0xe86b0002, 0xf86b0000, 0xf86b0001] {
            for mode in 0..3 {
                let mut obj = fixture(memory, 0x82004260, false)?;
                match mode {
                    0 => obj.blocked_relocation_targets.insert(
                        SectionAddress::new(1, 0x82004260),
                        SectionAddress::new(1, 0x82004268),
                    ),
                    1 => obj.blocked_relocation_sources.insert(
                        SectionAddress::new(0, 0x82100004),
                        SectionAddress::new(0, 0x82100008),
                    ),
                    _ => obj.sections[1].address += 0x1000,
                }
                let mut tracker = Tracker::new(&obj);
                tracker.process(&obj)?;
                tracker.apply(&mut obj, false)?;
                assert_eq!(obj.sections[0].relocations.iter().count(), 0);
            }
        }
        Ok(())
    }
