...
use crate::{
    ...
    obj::{
        ObjInfo, ObjKind, ObjReloc, ObjRelocations, ObjSection, ObjSectionKind, ObjSplit,
        ObjSymbol, ObjSymbolFlagSet, ObjSymbolFlags, ObjSymbolKind, ObjSymbolScope, ObjUnit,
        SectionIndex, SymbolIndex, section_kind_for_section,
    },
...
fn trim_split_alignment(obj: &mut ObjInfo) -> Result<()> {
    ...
    while let Some((section_index, section, addr, split)) = iter.next() {
        ...
        split_end = align_up(
            split_end,
            split.alignment(obj, section_index, section, addr),
        );
...
fn split_pdata(obj: &mut ObjInfo) -> Result<()> {
    // clear all the pdata splits now to avoid duplicates and false "overlaps with split" errors
    if let Some((idx, _)) = obj.sections.by_name(".pdata")?
        && let Some(sec) = obj.sections.get_mut(idx)
    {
        sec.splits.clear();
    }
    ...
    for (_section_index, section) in obj.sections.by_kind(ObjSectionKind::Code) {
        for (start_addr, split) in section.splits.iter() {
            ...
            for (i, chunks) in pdata_sec.data.as_chunks::<8>().0.iter().enumerate() {
...
fn resolve_link_order(obj: &ObjInfo) -> Result<Vec<ObjUnit>> {
    ...
    let mut iter = ordered_units
        .values()
        .filter_map(|unit| unit_to_index_map.get(unit.as_str()))
        .peekable();
...
pub fn split_obj(obj: &ObjInfo, module_name: Option<&str>) -> Result<Vec<ObjInfo>> {
    ...
    for (section_index, section) in obj.sections.iter() {
        ...
        loop {
            ...
            let next_addr = split_iter
                .peek()
                .map(|&(addr, _)| addr)
                .unwrap_or(section_end);
            if next_addr > split_end
                && section
                    .data_range(split_end.address, next_addr.address)?
                    .iter()
                    .any(|&b| b != 0)
            {
                bail!(
                ...
                .filter(|&(_, s)| s.section == Some(section_index))
            {
                ...
                let new_index = split_obj.symbols.add_direct(ObjSymbol {
                    ...
                    section: if split.common {
                        None
                    } else {
                        Some(out_section_idx)
                    },
                    size: symbol.size,
