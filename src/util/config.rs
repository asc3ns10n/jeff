...
pub fn parse_symbol_line(line: &str, obj: &mut ObjInfo) -> Result<Option<ObjSymbol>> {
    static SYMBOL_LINE: Lazy<Regex> = Lazy::new(|| {
        Regex::new(
            "^\\s*(?P<name>.+?)\\s*=\\s*(?:(?P<section>[A-Za-z0-9._]+):)?(?P<addr>[0-9A-Fa-fXx]+);(?:\\s*//\\s*(?P<attrs>.*))?$",
        )
        .unwrap()
    });
                ...
                match attr {
                    ...
                    "noreloc" => {
                        ...
                        obj.blocked_relocation_sources
                            .insert(addr, addr + symbol.size as u32);
    ...
    W: Write + ?Sized,
{
    ...
    for unit in obj
        .link_order
        .iter()
        .filter(|unit| all || !unit.autogenerated)
    {
        write!(w, "\n{}:", unit.name)?;
...
fn parse_section_line(captures: Captures, state: &SplitState) -> Result<SplitLine> {
    ...
    if let (Some(start), Some(end)) = (start, end) {
        ...
        Err(anyhow!(
            "Section '{}' missing start or end address",
            section.name
        ))
    }
}
    ...
    for result in r.lines() {
        ...
        match (&mut state, split_line) {
            (
                ...
                SplitLine::Unit(SplitUnit { name, order }),
            ) => {
...
impl SectionAddressRef {
    ...
    pub fn resolve(&self, obj: &ObjInfo) -> Result<SectionAddress> {
        let (section_index, section) = if let Some(section) = &self.section {
            ...
            bail!(
                "Section required for relocatable object address reference: {:#X}",
                self.address
            )
        };
        ensure!(
