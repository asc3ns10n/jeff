...
use crate::{
    ...
    obj::{
        ObjInfo, ObjKind, ObjRelocKind, ObjSections, ObjSymbolFlags, ObjSymbolKind, ObjUnit,
...
pub struct SplitArgs {
    ...
    #[argp(switch)]
    /// also emit address-ordered link objects under output/relink
    relink: bool,
}
...
fn split(args: SplitArgs) -> Result<()> {
    ...
    // dol split_write_obj
    let output_module = split_write_obj_exe(&mut exe, &config, &args.out_dir, args.relink)?;
...
fn split_write_obj_exe(
    ...
    if relink {
        let link_dir = out_dir.join("relink");
        let mut link_config = out_config.clone();
        link_config.units.clear();
        for object in crate::util::relink::link_objects(&module.obj, &split_objs)? {
            let path = link_dir.join("obj").join(coff_path_for_unit(&object.name));
            if let Some(parent) = path.parent() {
                DirBuilder::new().recursive(true).create(parent)?;
            }
            write_coff_if_changed(&path, &crate::util::xex::write_link_coff(&object)?)?;
            link_config.units.push(OutputUnit {
        ...
        let mut writer = buf_writer(&link_dir.join("config.json"))?;
        serde_json::to_writer_pretty(&mut writer, &link_config)?;
        writer.flush()?;
    }
...
fn load_analyze_xex(config: &ProjectConfig) -> Result<ExeAnalyzeResult> {
    ...
    // Analysis may recognize a signature at a hand-renamed address. Keep the
    // user's spelling (including a prior COFF qualification) authoritative.
    let configured_names: Vec<_> = obj
        .symbols
        .iter()
        .filter(|(_, symbol)| symbol.flags.0.contains(ObjSymbolFlags::NameFromConfig))
        .map(|(index, symbol)| (index, symbol.name.clone(), symbol.demangled_name.clone()))
        .collect();
    ...
    for (index, name, demangled_name) in configured_names {
...
fn disasm(args: DisasmArgs) -> Result<()> {
    ...
    for coff_obj in &split_objs {
        ...
        let coff_data = write_coff(coff_obj)?;
